...
Para obter uma lista detalhada das diferenças entre a versão anterior do OME e os novos recursos OME, consulte [Comparar versões de OME](ome-version-comparison.md).
...
As novas capacidades OME automaticamente estão habilitadas para a maioria das organizações do Office 365. Se sua organização não tiver OME habilitado automaticamente, ou se você ativou as novas capacidades OME desativado, siga estas etapas para configurar manualmente os novos recursos para o OME.
