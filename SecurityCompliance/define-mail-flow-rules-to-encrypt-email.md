...
No local os usuários podem enviar mensagens criptografadas usando OME se você rotear emails através do Exchange Online. Para fazer isso, você precisará configurar o email para o fluxo do seu servidor de email para o Office 365. Uma vez, você configurou email a fluir através do Office 365, e em seguida, você pode fazer as regras de fluxo de email para OME usando neste artigo.
...
4. No EAC, vá para **fluxo de email** \> **regras** \> ![novo ícone](media/457cd93f-22c2-4571-9f83-1b129bcfb58e.gif) ( **novo**) \> **criar uma nova regra**. Para obter mais informações sobre como usar o EAC, consulte o [Centro de administração do Exchange no Exchange Online](https://technet.microsoft.com/library/jj200743%28v=exchg.150%29.aspx).
...
4. No EAC, vá para **fluxo de email** \> **regras** \> **+** ( **novo**) \> **criar uma nova regra**. Para obter mais informações sobre como usar o EAC, consulte o [Centro de administração do Exchange no Exchange Online](https://technet.microsoft.com/library/jj200743%28v=exchg.150%29.aspx).
...
1. Use o Windows PowerShell no computador local para criar uma sessão PowerShell remota para o Exchange Online. Para obter mais informações, consulte [Connect to Exchange Online PowerShell](https://technet.microsoft.com/library/jj984289%28v=exchg.160%29.aspx).
    
2. Definir uma regra usando o cmdlet **New-TransportRule** e defina o atributo **ApplyOME** como **true**.
  ...
  - O parâmetro **- SentTo** , especifica uma condição que procura por destinatários em mensagens de email. Você pode usar qualquer valor que identifica exclusivamente o destinatário, como um endereço de email, nome, o nome diferenciado (DN) etc. Neste exemplo, o destinatário é identificado pelo endereço de email "DrToniRamos@hotmail.com". 
    
  - O parâmetro **- SentToScope** Especifica uma condição que procura o local de destinatários. Neste exemplo, a caixa de correio do destinatário é do hotmail e não é parte da organização do Office 365, portanto, o valor "NotInOrganization" é usado. 
    
    Para obter mais informações sobre as condições que você pode definir nas regras de fluxo de email usando este cmdlet, consulte [New-TransportRule](https://technet.microsoft.com/en-us/library/bb125138%28v=exchg.160%29.aspx).
...
#### <a name="to-create-a-rule-for-removing-encryption-from-email-replies-encrypted-without-the-new-ome-capabilities-by-using-the-eac"></a>Para criar uma regra para remover a criptografia das respostas de email criptografadas sem os novos recursos OME usando o EAC
<a name="DecryptruleEACNoNewOME"> </a>

1. Em um navegador da web, usando uma conta de trabalho ou da escola que recebeu permissões de administrador, [entrar no Office 365](https://support.office.com/article/Sign-in-to-Office-or-Office-365-b9582171-fd1f-4284-9846-bdd72bb28426#ID0EAABAAA=Web_browser).
    
2. Escolha os blocos de **Admin** . 
    
3. No Centro de administração do Office 365, escolha **Centros de Administração** \> **Exchange**.
    
4. No EAC, vá para **fluxo de email** \> **regras** \> **+** ( **novo**) \> **criar uma nova regra**. Para obter mais informações sobre como usar o EAC, consulte o [Centro de administração do Exchange no Exchange Online](https://technet.microsoft.com/en-us/library/jj200743%28v=exchg.150%29.aspx).
...
#### <a name="to-create-a-rule-to-remove-encryption-from-email-replies-encrypted-without-the-new-ome-capabilities-by-using-windows-powershell-for-exchange-online"></a>Para criar uma regra para remover a criptografia das respostas de email criptografadas sem os novos recursos OME usando o Windows PowerShell para Exchange Online
<a name="DecryptrulePShellNoNewOME"> </a>

1. Use o Windows PowerShell no computador local para criar uma sessão PowerShell remota para o Exchange Online. Para obter mais informações, consulte [Connect to Exchange Online PowerShell](https://technet.microsoft.com/library/jj984289%28v=exchg.160%29.aspx).
    
2. Definir uma regra usando o cmdlet **New-TransportRule** e defina o atributo **RemoveOME** como **true**.
  ...
  - O parâmetro **- SentToScope** Especifica uma condição que procura o local de destinatários. Neste exemplo, o valor "InOrganization" é usado que indica que: 
    
  - O destinatário é uma caixa de correio, usuário de email, grupo ou pasta pública habilitada para email na sua organização, ou
