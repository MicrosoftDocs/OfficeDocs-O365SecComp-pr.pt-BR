...
|**Campo do cabeçalho**|**Descrição**|
|:-----|:-----|
|spf|Descreve os resultados da verificação do SPF para a mensagem. Os valores possíveis incluem:<br/>• **aprovado (endereço IP)**: Indica que a verificação do SPF para a mensagem foi aprovada e inclui um endereço IP do remetente. O cliente está autorizado a enviar ou retransmitir emails em nome do domínio do remetente. <br/>• **falha (endereço IP)**: Indica a verificação do SPF para a mensagem reprovada e inclui um endereço IP do remetente. Isso também é conhecido como _falha grave_. <br/>• **softfail (motivo)**: Indica que o registro SPF designou o host como não tendo permissão para enviar, mas está em transição. <br/>• **neutro**: Indica que o registro SPF declarou explicitamente que não definiu se o endereço IP está autorizado. <br/>• **nenhum**: Indica que o domínio não tem um registro SPF ou o registro SPF não é avaliado como um resultado. <br/>• **temperror**: Indica que ocorreu um erro que pode ser temporário, por exemplo, um erro de DNS. Tentar novamente mais tarde poderá ser bem sucedido sem qualquer ação do administrador. <br/>• **permerror** indica que um erro permanente ocorreu. Isso acontece quando, por exemplo, o domínio tem um registro SPF mal formatado.|
